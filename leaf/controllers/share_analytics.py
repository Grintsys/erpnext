...
def log_share_event(doctype, name, print_format=None, share_method=None):
	...
	# Validate read permission on target document
...
def download_pdf(doctype, name, format=None, no_letterhead=0, letterhead=None):
	...
	and wkhtmltopdf ConnectionRefusedError / HostNotFoundError in single-threaded servers.
	"""
	if not doctype or not name:
		frappe.throw(_("DocType and Name are required."))

	# Ensure assets_json is loaded in frappe.local context
	...
	# Replace relative and HTTP URLs with local file:// paths for wkhtmltopdf
