...
def execute(filters=None):
	...
	return AccountReceivable3Report(filters).run(args)

class AccountReceivable3Report(object):
	...
	def customer_documents(self):
		...
		for document in documents:
			...
			row = [document.posting_date, document.customer, sales_partner,"","Customer Documents", document.name, document.document_number, document.total, paid, document.outstanding_amount, 0, "HNL"]
	...
	def set_invoice_details(self, row):
		...
		if row.voucher_type == 'Sales Invoice':
			...
			row.sales_partner = frappe.db.get_value(
				"Sales Invoice",
				row.voucher_no,
				"sales_partner"
			)

			if not row.sales_partner:
				customer_invoice = frappe.db.get_value(
					"Sales Invoice",
					row.voucher_no,
					"customer"
				)
